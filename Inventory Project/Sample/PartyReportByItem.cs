...
namespace sample
{
...

    {
        ...
        private void guna2ComboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                string Query = string.Format("(select P.TableName, PI.ItemName, P.BillNo as Number,P.DeleteData,P.Company_ID, PI.Qty, PI.freeQty, PI.ItemAmount from tbl_PurchaseBill as P inner join tbl_PurchaseBillInner as PI on P.ID = PI.ID where  PartyName = '{0}'  and P.Company_ID='" + NewCompany.company_id + "' and P.DeleteData='1' union all select P.TableName, PI.ItemName, P.OrderNo as Number,P.DeleteData,P.Company_ID, PI.Qty, PI.freeQty, PI.ItemAmount from tbl_PurchaseOrder as P inner join tbl_PurchaseOrderInner as PI on P.ID = PI.ID where  PartyName = '{0}'  and P.Company_ID='" + NewCompany.company_id + "' and P.DeleteData='1') union all (select P.TableName, PI.ItemName, P.OrderNo as Number,P.DeleteData,P.Company_ID, PI.Qty, PI.freeQty, PI.ItemAmount from tbl_SaleOrder as P inner join tbl_SaleOrderInner as PI on P.ID = PI.ID where  PartyName = '{0}'  and P.Company_ID='" + NewCompany.company_id + "' and P.DeleteData='1' union all select P.TableName,P.DeleteData,P.Company_ID, PI.ItemName, P.InvoiceID as Number, PI.Qty, PI.freeQty, PI.ItemAmount from tbl_SaleInvoice as P inner join tbl_SaleInvoiceInner as PI on P.ID = PI.ID where  PartyName = '{0}'  and P.Company_ID='" + NewCompany.company_id + "' and P.DeleteData='1')", cmbparty.Text);
                DataSet ds = new DataSet();
                SqlDataAdapter da = new SqlDataAdapter(Query, con);
                da.Fill(ds, "temp");
                dgvPartyReport.DataSource = ds;
