...
namespace sample
{
    public partial class CashInHandHomepage : UserControl
    {
        ...
        private void bindbankdata()
        {
            ...
            SqlCommand cmd = new SqlCommand("(select TableName as Type,PartyName,InvoiceDate as Date,Total from tbl_SaleInvoice)union all(select TableName as Type,PartyName,OrderDate as Date,Total from tbl_SaleOrder )Union all(select TableName as Type,PartyName,BillDate as Date,Total from tbl_PurchaseBill )Union all(select TableName as Type,PartyName,OrderDate as Date,Total from tbl_PurchaseOrder)union all(select TableName as Type,CustomerName as PartyName,Date as Date,TOTAL as Total from tbl_PaymentIn )union all(select TableName as Type,CustomerName as PartyName,Date as Date,TOTAL as Total  from tbl_Paymentout)Union all(select CashAdjustment as Type,Description as PartyName,Date as Date,CashAmount from tbl_CashAdjustment) where Company_ID='"+NewCompany.company_id+"' and DeleteData='1'", con);
