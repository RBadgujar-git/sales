    ...
    public partial class CreditNote : Form
    {
       ...
       //     quantitcheek();
        ...
        private void insert_record_inner(string id)
        {
            for (int i = 0; i < dgvInnerCreditNote.Rows.Count; i++)
            {
                try
                {
                    ...
                    cmd.Parameters.AddWithValue("@CalTotal", dgvInnerCreditNote.Rows[i].Cells["CalTotal"].Value.ToString());
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Same Record Not Intert");
            }
        }
           ...
           // {
                try
                {
                    ...
                    float TA = 0, TD = 0, TGST = 0, dis1 = 0, tax = 0, itotal = 0;
                        ...
                        dgvInnerCreditNote.Rows[row].Cells[13].Value = cgst;
                        dgvInnerCreditNote.Rows[row].Cells[14].Value = sgst;
                        dgvInnerCreditNote.Rows[row].Cells[15].Value = igst;
                        dgvInnerCreditNote.Rows[row].Cells[16].Value = caltotal;
                    ...
                    for (int i = 0; i < dgvInnerCreditNote.Rows.Count; i++)
                    {
                        dis1 += float.Parse(dgvInnerCreditNote.Rows[i].Cells["Discount_Amount"].Value?.ToString());
                        textBox4.Text = dis1.ToString();
                        tax += float.Parse(dgvInnerCreditNote.Rows[i].Cells["Tax_Amount"].Value?.ToString());
                        textBox3.Text = tax.ToString();
                        itotal += float.Parse(dgvInnerCreditNote.Rows[i].Cells["CalTotal"].Value?.ToString());
       ...
       // }
        ...
        private void Print_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("DO YOU WANT PRINT??", "PRINT", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                {
                    DataSet ds = new DataSet();
                    string Query = string.Format("SELECT a.AddLogo,a.GSTNumber,a.CompanyID,a.CompanyName, a.Address, a.PhoneNo, a.EmailID,a.AdditinalFeild1,a.AdditinalFeild2,a.AdditinalFeild3,b.PartyName,b.BillingName,b.ContactNo, b.ReturnNo, b.InvoiceDate, b.DeliveryLocation,b.DeliveryDate,b.DueDate, b.Tax1, b.TaxAmount1,b.TotalDiscount,b.DiscountAmount1,b.Total,b.Received,b.RemainingBal,c.ID,c.BasicUnit,c.ItemName,c.ItemCode,c.SalePrice,c.Qty,c.CGST, c.SGST,c.IGST,c.SaleTaxAmount,c.TaxForSale,c.ItemAmount FROM tbl_CompanyMaster  as a, tbl_CreditNote1 as b,tbl_CreditNoteInner as c where b.ReturnNo='{0}' and c.ReturnNo='{1}' and a.CompanyID='" + NewCompany.company_id + "' ", txtReturnNo.Text, txtReturnNo.Text);
                    SqlDataAdapter SDA = new SqlDataAdapter(Query, con);
                    SDA.Fill(ds);
