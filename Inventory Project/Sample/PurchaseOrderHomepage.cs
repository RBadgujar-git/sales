        ...
        private void txtFilterBy_TextChanged(object sender, EventArgs e)
        {
            if (txtFilterBy.Text != "")
            {
                try
                {
                    ...
                    string Query = string.Format("select OrderNo,PartyName,ContactNo,OrderDate,Paid,Total,RemainingBal,Status from tbl_PurchaseOrder where PartyName like '%{0}%' and  Company_ID='" + NewCompany.company_id + "' and DeleteData='1'", txtFilterBy.Text);
                    DataSet ds = new DataSet();
                    SqlDataAdapter da = new SqlDataAdapter(Query, con);
                    da.Fill(ds, "temp");
                    dgvPurchaseOrder.DataSource = ds;
