...
namespace sample
{
    public partial class ExpenseHomePage : UserControl
    {
        ...
        private void dgvcategory_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            con.Open();
            DataTable dt = new DataTable();
            SqlCommand Query1 = new SqlCommand("select Date,ExpenseCategory,Total,Paid,Balance from tbl_Expenses where ExpenseCategory like '%{0}%' and Company_ID='" + NewCompany.company_id + "' and DeleteData='1'", con);
        ...
        private void dtpTo_ValueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                string SelectQuery = string.Format("(select Date, ExpenseCategory, Total,Paid,Balance from tbl_Expenses where Date between '" + dtpFrom.Value.ToString(sysUIFormat) + "' and '" + dtpTo.Value.ToString(sysUIFormat) + "' and ExpenseCategory='" + lblCategory.Text + "' and Company_ID='" + NewCompany.company_id + "' and DeleteData='1')");
                DataSet ds = new DataSet();
